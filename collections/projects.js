Meteor.methods({
	...
	addToProject: function (dataset) {
		...
		if (typeof(projectDocs) !== 'undefined') {
			projectDocsArrayLength = projectDocs.length;
			for (j = 0; j < projectDocsArrayLength; j++) {
				...
				if (dataset.docId === projectDocs[j].docId) {
					...
					docAlreadyExistsInThisProject = true;
				}
			}
		}
			...
			Projects.update(
				...
				},
				{
				    $push: {
							docs: {
								docId: dataset.docId,
								lastRevision: dataset.lastRevision,
								dateAdded: new Date(),
							},
						},
				},
